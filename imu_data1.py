...
def opt1():
    ...
    # print("Acceleration_x:" +  str(accel_x),flush=True)
    # print("Acceleration_y:" +  str(accel_y),flush=True)
    # print("Acceleration_z:" +  str(accel_z),flush=True)

    ang_x_val.append(ang_x)
    ang_y_val.append(ang_y)
    ang_z_val.append(ang_z)
    
    # print("Angular_velocity_x:" + str(ang_x),flush=True)
    # print("Angular_velocity_y:" + str(ang_y),flush=True)
    # print("Angluar_velocity_z:" + str(ang_z),flush=True)
    ...
    #print("Magnetometer_x:" + str(mag_x),flush=True)
    #print("Magnetometer_y:" + str(mag_y),flush=True)
    #print("Magnetometer_z:" + str(mag_z),flush=True)
    ...
    port = 8889  # Replace with the server's port number
    message = f"{accel_x},{accel_y},{accel_z},{ang_x},{ang_y},{ang_z},{mag_x},{mag_y},{mag_z}"
...
def opt5():
    ...
    # with open("Kaju_Badem_data3", "a") as file:
    #     file.write(f"accel_x = {del_accel_x}" + "\n" + f"accel_y = {del_accel_y}" + "\n"
    #                + f"accel_z = {del_accel_z}" + "\n" + f"ang_x = {del_ang_x}" + "\n"
    #                + f"ang_y = {del_ang_y}" + "\n" + f"ang_z = {del_ang_z}" + "\n"
    #                + f"mag_x = {mag_x}" + "\n" + f"mag_y = {mag_y}" + "\n"
    #                + f"mag_z = {mag_z}" + "\n" + "There is no error: " + str(check_sum(data))
    #                + "\n" + f"time = {sample_time}" + "\n")
    
    if max(abs(del_ang_x), abs(del_ang_y), abs(del_ang_z))>0.02:
        print('Shower head has fallen!')
