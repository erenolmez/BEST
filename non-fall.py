...
for fn in glob.glob(r'C:\Users\HP\PycharmProjects\485_PBC\*.npy'):
    ...
    if 'stand' in fn:
        ...
        # plt.figure()
        # plt.plot(time_instances, o)
        #
        # plt.figure()
        # plt.plot(time_window, o_window)
        # plt.figure()
        # window = signal.hamming(1280)
        # plt.plot(window)
        ...
        # plt.figure()
        # plt.pcolormesh(np.linspace(0, 10, 25728), freq[20:], np.abs(out[20:][:]))
        # plt.ylabel('Frequency [Hz]')
        # plt.xlabel('Time [sec]')
        #plt.show()
n = np.array(n)
print(n)
#save('data_1_detectable_freq.npy', n)